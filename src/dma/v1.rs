...
unsafe fn dma_on_irq(r: pac::dma::Dma, mux_num_base: u32) {
    ...
    if err != 0 {
        panic!("DMA: error on DMA@{:08x}, errsts=0b{:08b}, tc=0b{:08b}, abort=0b{:08b}", r.as_ptr() as u32, err, tc, abort);
    }
...
impl<'a, W: Word> ReadableRingBuffer<'a, W> {
    ...
    pub unsafe fn new(
        ...
        // Read back the correctly-configured ctrl value from hardware,
        // then set enable bit for the descriptor (so DMA restarts on reload)
        let ctrl_val = ch_cr.ctrl().read().0 | 1; // enable=1
