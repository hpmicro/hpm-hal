...
impl MachineTimerDriver {
    fn init(&'static self) {
        ...
        let cnt_per_second = crate::sysctl::clocks().mchtmr0.to_Hz() as u64;
        let cnt_per_tick = cnt_per_second / embassy_time_driver::TICK_HZ;

        //  crate::println!("cnt per tick: {}", cnt_per_tick); , 24
        ...
        mchtmr.mtimecmp().write(|w| unsafe { w.mtimecmp().bits(u64::MAX - 1) });
    }
...
impl embassy_time_driver::Driver for MachineTimerDriver {
    fn now(&self) -> u64 {
        let mchtmr = unsafe { &*pac::MCHTMR::PTR };
        mchtmr.mtime().read().bits() / self.period.load(Ordering::Relaxed) as u64
    }
    ...
    fn set_alarm(&self, alarm: AlarmHandle, timestamp: u64) -> bool {
        critical_section::with(|cs| {
            ...
            mchtmr
                .mtimecmp()
                .write(|w| unsafe { w.mtimecmp().bits(safe_timestamp) });
            unsafe {
