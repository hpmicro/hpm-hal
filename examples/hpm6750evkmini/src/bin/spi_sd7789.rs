...
fn main() -> ! {
    ...
    let cs = Output::new(p.PB24, Level::High, Speed::Fast);

    let spi_config = Config {
        frequency: Hertz(80_000_000),
        ..Default::default()
    };
    let spi: hal::spi::Spi<'_, Blocking> = Spi::new_blocking_txonly(p.SPI2, p.PB21, p.PB22, spi_config);
    let spi = AtomicCell::new(spi);
    let spi_dev = AtomicDevice::new_no_delay(&spi, cs).unwrap();

    let dc = Output::new(p.PE26, Level::High, Speed::Fast);
    let mut display = ST7789::<_, _, 240, 240, 0, 0>::new(spi_dev, dc);
