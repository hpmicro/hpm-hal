//! PDM FFT Spectrum Visualizer — SSD1306 128×64 OLED
//!
//! Architecture: single async loop with blocking peripherals
//! - blocking PDM read (no DMA) to acquire one FFT frame
//! - blocking I2C writes to update SSD1306 OLED
//!
//! Hardware:
//! - PDM mic: PY10 (CLK) + PY11 (DATA)
//! - I2C OLED: PB11 (SCL) + PB10 (SDA)
...
use hpm_hal as hal;
use hpm_hal::pdm::{extract_sample, ChannelMask, Config as PdmConfig, Pdm};
...
impl SSD1306 {
    ...
    pub fn init(&mut self) {
        ...
        self.cmd(SETLOWCOLUMN | ((HEIGHT as u8) - 1));
        ...
        self.cmd(SETPRECHARGE);
        self.cmd(0xF1);
        for &c in &[
            SETVCOMDETECT, 0x40, DISPLAYALLON_RESUME, NORMALDISPLAY, DEACTIVATE_SCROLL, DISPLAYON,
        ] {
            self.cmd(c);
        }
    }
    ...
    pub fn display_fb(&mut self, fb: &[u8]) {
        self.cmd(cmds::PAGEADDR);
        self.cmd(0);
        self.cmd(0xFF);
        self.cmd(cmds::COLUMNADDR);
        self.cmd(0);
        ...
        for chunk in fb.chunks(32) {
            ...
            self.i2c.blocking_write(self.addr, &buf[..1 + chunk.len()]).unwrap();
        }
    }
}
...
const FONT: [[u8; 6]; 36] = [
    ...
    [0b0111, 0b1000, 0b1000, 0b1011, 0b1001, 0b0110], // G
    [0b1001, 0b1001, 0b1111, 0b1001, 0b1001, 0b1001], // H
    [0b0111, 0b0010, 0b0010, 0b0010, 0b0010, 0b0111], // I
    [0b0001, 0b0001, 0b0001, 0b0001, 0b1001, 0b0110], // J
    ...
    [0b1110, 0b1001, 0b1001, 0b1110, 0b1000, 0b1000], // P
    [0b0110, 0b1001, 0b1001, 0b1011, 0b0110, 0b0001], // Q
    [0b1110, 0b1001, 0b1001, 0b1110, 0b1010, 0b1001], // R
    [0b0111, 0b1000, 0b0110, 0b0001, 0b0001, 0b1110], // S
    [0b0111, 0b0010, 0b0010, 0b0010, 0b0010, 0b0010], // T
    ...
    [0b1001, 0b1001, 0b0110, 0b0010, 0b0010, 0b0010], // Y
...
const FREQ_PER_BIN: u32 = SAMPLE_RATE / FFT_SIZE as u32; // 31.25 Hz/bin
...
async fn main(_spawner: Spawner) -> ! {
    ...
    info!("PDM FFT OLED — HPM6750EVKMINI");
    info!("FFT={} pts, {}.{} Hz/bin", FFT_SIZE, FREQ_PER_BIN, (SAMPLE_RATE * 100 / FFT_SIZE as u32) % 100);

    let mut led = Output::new(p.PB19, Level::Low, Speed::default());

    // ── I2C OLED (blocking, no DMA) ──
    let mut i2c_config = hal::i2c::Config::default();
    i2c_config.mode = hal::i2c::I2cMode::FastPlus;
    let i2c = I2c::new_blocking(p.I2C0, p.PB11, p.PB10, i2c_config);
    ...
    pdm_config.channels = ChannelMask(0x01); // Ch0 mono
    let mut pdm = Pdm::new(p.PDM, p.I2S0, p.PY10, p.PY11, pdm_config);
    ...
    loop {
        ...
        for bar in 0..NUM_BARS {
            ...
            for b in 0..BINS_PER_BAR {
                let bin = bin_start + b;
                if bin == 0 || bin >= FFT_SIZE / 2 { continue; }
        ...
        if (now - last_log).as_secs() >= 1 {
            info!("fps={} peak={}Hz", frames, peak_freq);
            frames = 0;
            last_log = now;
        }
